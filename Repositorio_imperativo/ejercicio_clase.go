...
func masParecido(lista [5]persona, muestra [10]byte) persona {
	cont, max := 0, 0
	var masparecido persona
	for _, individuo := range lista {
		cont = 0
...
func (person *persona) verificar(muestra [10]byte) int {// person es la variable y *persona es el tipo (referencia puntero)
	cont := 0
...
func masParecido3(lista [5]persona, muestra [10]byte) persona {// tarea cambier a persona 
	cont, max := 0, 0
	var masparecido persona
	for _, individuo := range lista {
